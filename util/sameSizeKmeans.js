...
var log = Math.log, //2.302585092994046=Math.log(10) 返回一个数的自然对数
	pow = Math.pow, //81=Math.pow(3,4)
	sqrt = Math.sqrt, //3=Math.sqrt(9)
	round = Math.round, // 函数返回一个数字四舍五入后最接近的整数
	floor = Math.floor, //像下取整
	ceil = Math.ceil, //像上取整
	random = Math.random, // 伪随机数在范围[0，1)，包括0不包括1
	...
	sortBy = function (a, b, c) {//根据某一个值排序
		c = a.slice();
...
function createMatrixOfDistance(input) {
	...
	input.forEach((itemFrom) => {
		...
		input.forEach((itemTo) => {
			distances.push(distance(itemFrom, itemTo));
		});
